...
class GiftForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["gifts"].choices = list(
            map(
                lambda g: (g.id, g.name),
                Gift.objects.filter(Q(user=None) | Q(user=user)),
            )
        )
        self.fields["gifts"].initial = list(
            map(lambda g: g.id, Gift.objects.filter(user=user))
        )
